...
i = 0
with open(TRAINER_CONSTANTS_FILE) as f:
    for line in f:
        ...
        if skip: # empty index
            ...
            # remove suffix from psychic trainer name
            if re.search("PSYCHIC", name):
                name = name.replace("_TR", "")
...
i = 0
with open(TRAINER_AI_FILE) as f:
    for line in f:
        collect = re.search(pattern, line)
        if collect:
            i += 1
...
# annoyingly this file uses a different format for the class names
...
with open(TRAINER_PARTIES_FILE) as f:
    for line in f:
        ...
        if new_class:
            ...
            for i in range(0, len(chunks), 2):
                party.append((int(chunks[i]), chunks[i+1]))
            parties[name].append(party)

        else:
            continue

# record the number of parties
...
# pickle
trainer_data_filename = PICKLE_PATH + "trainer_data" + PICKLE_EXT
trainer_data.to_pickle(trainer_data_filename, compression = None, protocol = pickle.DEFAULT_PROTOCOL)
print("Wrote to", trainer_data_filename)

trainer_classes_filename = PICKLE_PATH + "trainer_classes" + PICKLE_EXT
with open(trainer_classes_filename, "wb") as f:
    pickle.dump(trainer_classes, f)
print("Wrote to", trainer_classes_filename)
